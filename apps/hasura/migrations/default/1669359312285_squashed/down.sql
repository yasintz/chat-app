
alter table "public"."channel" alter column "deleted_at" set not null;

-- Could not auto-generate a down migration.
-- Please write an appropriate down migration for the SQL below:
-- alter table "public"."member" add column "deleted_at" timestamptz
--  null;

-- Could not auto-generate a down migration.
-- Please write an appropriate down migration for the SQL below:
-- alter table "public"."member" add column "updated_at" timestamptz
--  not null default now();
--
-- CREATE OR REPLACE FUNCTION "public"."set_current_timestamp_updated_at"()
-- RETURNS TRIGGER AS $$
-- DECLARE
--   _new record;
-- BEGIN
--   _new := NEW;
--   _new."updated_at" = NOW();
--   RETURN _new;
-- END;
-- $$ LANGUAGE plpgsql;
-- CREATE TRIGGER "set_public_member_updated_at"
-- BEFORE UPDATE ON "public"."member"
-- FOR EACH ROW
-- EXECUTE PROCEDURE "public"."set_current_timestamp_updated_at"();
-- COMMENT ON TRIGGER "set_public_member_updated_at" ON "public"."member"
-- IS 'trigger to set value of column "updated_at" to current timestamp on row update';

-- Could not auto-generate a down migration.
-- Please write an appropriate down migration for the SQL below:
-- alter table "public"."member" add column "created_at" timestamptz
--  not null default now();

-- Could not auto-generate a down migration.
-- Please write an appropriate down migration for the SQL below:
-- alter table "public"."file" add column "deleted_at" timestamptz
--  null;

ALTER TABLE "public"."message_reaction" ALTER COLUMN "id" drop default;

DROP TABLE "public"."message_reaction";

DROP TABLE "public"."reaction";

DROP TABLE "public"."message_file";

alter table "public"."message" drop constraint "message_reply_to_id_fkey";

alter table "public"."message" drop constraint "message_parent_id_fkey";

DROP TABLE "public"."message";
